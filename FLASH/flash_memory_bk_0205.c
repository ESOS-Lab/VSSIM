...
void FLASH_STATE_CHECKER(void)
{
	...
	for(i=0; i<N_FLASH; i++){
		for(j=0; j<N_PLANES_PER_FLASH; j++){
			...
			cur_plane = &flash[i].plane[j];
			cmd = cur_plane->cmd;

			/* Get registers to update */
			data_reg = &cur_plane->data_reg;
			page_cache = &cur_plane->page_cache;

			/* Update data register state of this plane */
			UPDATE_DATA_REGISTER(cur_plane, data_reg, cmd, channel_nb, t_now);

			if(PAGE_CACHE_REG_ENABLE){
				/* Update page cache register state of this plane */
				UPDATE_PAGE_CACHE_REGISTER(cur_plane, page_cache, data_reg, 
						cmd, channel_nb, t_now);
			}
		}
	}
}
...
void UPDATE_DATA_REGISTER(plane* cur_plane, reg* data_reg, uint8_t cmd, 
				int channel_nb, int64_t t_now)
{
...
void UPDATE_PAGE_CACHE_REGISTER(plane* cur_plane, reg* page_cache, reg* data_reg, 
				uint8_t cmd, int channel_nb, int64_t t_now)
{
