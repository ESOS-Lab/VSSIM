...
// 16-Sep-2017: Added by Jinsoo Yoo
#include "vssim.h"
#include "ssd.h"
...
#ifdef VSSIM_NVME
    event_queue_entry* vssim_event = req->vssim_event;
    END_SSD_NVME_RW(vssim_event);
    req->vssim_event = NULL;
#endif
...
// 09-Jan-2018: Added by Jinsoo Yoo
#ifdef VSSIM_NVME
static uint16_t nvme_discard(NvmeCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
    NvmeRequest *req)
{
    uint32_t nr;
    ...
    switch (le32_to_cpu(cmd->cdw11)) {	// cmd->dsm.attributes
    case NVME_DSMGMT_AD: 
        if(IS_SSD_TRIM_ENABLED()){
            nr = le32_to_cpu(cmd->cdw10);
            SSD_DSM_DISCARD(req, nr);
        }
        return NVME_NO_COMPLETE;
...
#ifdef VSSIM_NVME
    case NVME_CMD_DSM:
        return nvme_discard(n, ns, cmd, req);
#endif
    default:
        return NVME_INVALID_OPCODE | NVME_DNR;
    }
